...
**Commits:** `b428ae1` (S4-1), `be6574e` (S4-2)
**Estado do processo:** PID 1611266, subido 26/08 21:49 SP, rodando `dd451c9`.
**Os dois commits desta sprint NÃO estão no ar** — ver §5.
...
É migração, é seguro (o tipo é novo, zero linhas existentes), e **está PROPOSTO, não
aplicado** — aguarda CHECKPOINT.

---

## 3. Aliases — a tabela para aprovação (NADA foi executado)
...
### 3.4 O INSERT — pronto, **NÃO executado**, aguardando sua aprovação

```sql
-- SOMENTE apos aprovacao da tabela do §3.2. Nada aqui foi rodado.
...
| Erica / Amanda Pavão transferidas? | **NÃO.** Seguem em `aguardando_ano` e `aguardando_atuacao`, `transferido_motivo` NULL, `updated_at` de 26/08 09:36 e 09:01. O SDR não digitou nada. |
...
### 4.2 O que o dia 27/08 ainda precisa responder
...
## 5. O que falta para o Sprint 4 estar NO AR

**Os dois commits estão no repo e NÃO no processo.** O PID 1611266 roda `dd451c9`. O S4-2
acrescenta uma task ao `lifespan` (`agente_parado_job`), que só existe depois de um restart.

**Não reiniciei** — a regra é avisar antes. O momento é favorável: 22:50 SP, zero tráfego
desde 19:18, nenhuma ação vencendo antes de 27/08 08:30 (o lembrete 152 da Amanda Pavão).

**Aguardo o seu ok para reiniciar.**
...
* **Aprovação da tabela de aliases** (§3.2) e decisão sobre os 5 pendentes (§3.3).
* **CHECKPOINT do índice único parcial** de `agente_parado` (§2.6).
* Os dois intercâmbios (339 leads) — se não são pós, é buraco de **admissão** (§3.3).
